"""Requesty model registry for managing model configurations and aliases."""
...
class RequestyModelConfig:
    """Configuration for a Requesty model."""
...
class RequestyModelRegistry:
    """Registry for managing Requesty model configurations and aliases."""
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.model_map: dict[str, RequestyModelConfig] = {}  # model_name -> config
        ...
        if config_path:
            ...
            env_path = os.getenv("REQUESTY_MODELS_CONFIG_PATH")
            if env_path:
                # Environment variable path - translate for Docker if needed
                translated_path = translate_path_for_environment(env_path)
                self.config_path = Path(translated_path)
            else:
                # Default to conf/requesty_models.json
                self.config_path = Path(__file__).parent.parent / "conf" / "requesty_models.json"
    ...
    def reload(self) -> None:
        ...
        try:
            ...
            logging.info(f"Loaded {len(self.model_map)} Requesty models with {len(self.alias_map)} aliases")
        except ValueError as e:
            # Re-raise ValueError only for duplicate aliases (critical config errors)
            logging.error(f"Failed to load Requesty model configuration: {e}")
    ...
    def _read_config(self) -> list[RequestyModelConfig]:
        ...
        if not self.config_path.exists():
            logging.warning(f"Requesty model config not found at {self.config_path}")
            return []
        ...
        try:
            ...
            for model_data in data.get("models", []):
                config = RequestyModelConfig(**model_data)
    ...
    def _build_maps(self, configs: list[RequestyModelConfig]) -> None:
    ...
    def resolve(self, name_or_alias: str) -> Optional[RequestyModelConfig]:
